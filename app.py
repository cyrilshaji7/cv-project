...
image_save_interval = 5  # in seconds, default value
is_training = False
...
def detection_loop():
    global is_detecting, is_training, label_counter, image_save_interval, last_detection_time
    while is_detecting:
...
def generate_frames():
    global is_training, last_detection_time
    while True:
        success, frame = cap.read()
        if not success:
            break
...
def start_detection():
    global is_detecting, detection_thread, last_detection_time
    with training_lock:
        if not is_detecting:
            if is_training:
